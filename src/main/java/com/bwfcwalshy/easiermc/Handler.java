...
import com.bwfcwalshy.easiermc.itemsandblocks.blocks.machines.SolarPanel;
...
public class Handler {
    ...
    public void registerRecipes() {
        for (ItemCategory category : registery.keySet()) {
            for (EasierMCBase base : registery.get(category)) {
                if (base.getRecipe() != null && !(base.getRecipe() instanceof AdvancedRecipe))
                    Bukkit.addRecipe(base.getRecipe());
                else if (base.getRecipe() != null && base.getRecipe() instanceof AdvancedRecipe)
                    this.advancedRecipes.add((AdvancedRecipe) base.getRecipe());
            }
        }
    }
    ...
    public Map<Location, BlockBase> getBlocks(String simpleName) {
        ...
        blocks.forEach((location, blockBase) -> {
            if (blockBase.getSimpleName().equalsIgnoreCase(simpleName)) blockBases.put(location, blockBase);
        });
        return blockBases;
    }
            ...
            if (compare.hasItemMeta()) {
                if (!toCheck.hasItemMeta()) return false;
                    ...
                    if (compare.getItemMeta().hasDisplayName()) {
                        ...
                        else if (!toCheck.getItemMeta().getDisplayName().equals(compare.getItemMeta().getDisplayName()))
                            return false;
                    }
